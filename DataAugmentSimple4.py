...
#############################################################################################
#Generator
dataTrainGen = ImageDataGenerator(horizontal_flip=True,vertical_flip=True)
dataTestGen = ImageDataGenerator()
#Fitting
dataTrainGen.fit(x_train)
dataTestGen.fit(x_test)

# Fitting the CNNmodel 
#result = CNNModel.fit(x_train, y_train, batch_size=32, validation_data=(x_test,y_test), epochs=3, verbose=1)
result = CNNModel.fit_generator(dataTrainGen.flow(x_train, y_train, batch_size=32),steps_per_epoch = len(x_train), epochs = 2, 
					validation_data = dataTestGen.flow(x_test, y_test, batch_size = 32),validation_steps = len(x_test))
#calculating scores
...
plt.ylabel('Accuracy')
plt.title('Training Accuracy & Test Accuracy vs No. of Epochs')
...
plt.xlabel('Number of Epochs')
plt.ylabel('Loss')
plt.title('Training Loss & Test Loss vs No. of Epochs')
plt.legend(['train', 'test'], loc='upper left')
plt.grid()
plt.show()