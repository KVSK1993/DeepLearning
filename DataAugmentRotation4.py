...
for d in degreeValues:
    ...
    dataTestGen.fit(testCreateRotated)
    score = CNNModel.evaluate_generator(dataTestGen.flow(testCreateRotated, y_test, batch_size = 32), steps = 1000)
...
plt.title('TestSet: Rotation (Accuracy) ')
plt.grid()
plt.show()
...
plt.title('TestSet: Rotation (Loss) ')
plt.grid()
plt.show()
